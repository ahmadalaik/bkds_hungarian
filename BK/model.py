...
df_col_selected = df.iloc[:, [1, 2, 7, 8, 10, 14, 17, 30, 36, 38, 39, 42, 49, 56]]
column_mapping = {
    2: 'age',
    3: 'sex',
    8: 'cp',
...
df_col_selected.rename(columns=column_mapping, inplace=True)

columns_to_drop = ['ca', 'slope', 'thal']
df_col_selected = df_col_selected.drop(columns_to_drop, axis=1)

meanTBPS = df_col_selected['trestbps'].dropna()
meanChol = df_col_selected['chol'].dropna()
meanfbs = df_col_selected['fbs'].dropna()
meanRestCG = df_col_selected['restecg'].dropna()
meanthalach = df_col_selected['thalach'].dropna()
meanexang = df_col_selected['exang'].dropna()
...
df_clean = df_col_selected.fillna(value=fill_values)
...
model = pickle.load(open("model/knn_model", "rb"))
...
with tab1:
    ...
    trestbps = st.sidebar.number_input(label=":violet[**Resting Blood Pressure** (in mmHg on admission to the hospital)]", min_value=df_final['trestbps'].min(), max_value=df_final['trestbps'].max())
...
with tab2:
    ...
    st.write("")
    st.download_button("Download CSV Example", data=sample_csv, file_name='sample_heart-disease_parameters.csv', mime='text/csv')
